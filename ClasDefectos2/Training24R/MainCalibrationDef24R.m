...
clc; clear all; close all;
 ...
 pathEntradaSamples=strcat(pathPrincipal,'inputSamples/');

 pathConfiguracion=strcat(pathPrincipal,'conf/');

 pathCalibracion=strcat(pathPrincipal,'calibration/'); %se utiliza para situar las imagenes de calibracion
 pathCalibracionSiluetas=strcat(pathCalibracion,'sFrutas/');
...
%% Inicio del proceso
 if(banderaCalibracion==NOCALIBRADO)
    ...
    %ProcesarImgCalibracion(pathEntradaCalibracion, pathCalibracion, nombreImagenP, archivoConfiguracion);
    ...
    calibracionDef24R(pathCalibracionSiluetas, nombreImagenP);
...
%ProcesarImgCalibracion(pathEntradaCalibracion, pathCalibracion, nombreImagenP, ArrayCuadros, areaObjetosRemoverBR, canalLMin, canalLMax, canalAMin, canalAMax, canalBMin, canalBMax);
        ...
        %ProcesarImagen( corrida, pathPrincipal, pathEntradaSamples, pathConfiguracion, pathCalibracion, pathResultados, banderaCalibracion, banderaRotar, nombreImagenP, archivoConfiguracion, archivoCalibracion, archivoVector );
        %break
    end %for del listado
