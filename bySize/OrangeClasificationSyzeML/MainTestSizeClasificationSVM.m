...
% la eficacia
 ...
 nombreArchivoSetCompleto='aTSCompletoSizeML.csv'; %el mismo conjunto etiquetado que se usó en KNN
 
 
 %Leer las 
 nombreArchivoTraining='aTrSizeMLSVM.csv';
 nombreArchivoTest='aTestSizeMLSVM.csv';
nombreArchivoScreen='screenManchasSSVMCF.txt'; 
...
%se cargan las etiquetas de clasificacion, el lugar 12 corresponde a la
%etiqueta de clasificacion
tablaDSTrainingClasificacion=tablaDSTraining(:,12);

% se cargan las caracteristicas que alimentaran al clasificador, el lugar 7
% corresponde a la etiqueta de diametro en milímetros, 11 y 12 eje mayor y eje
% menor en milímetros
...
%        fprintf('cs=> %s \n',char(clasificacionObjeto(1)));
        ...
        fprintf('%s, ejeMayorSoft_mm = %f, ejeMenorSoft_mm = %f',nombreDelaImagenComparar, double(objetoComparar(1,1)), double(objetoComparar(1,2)));        
