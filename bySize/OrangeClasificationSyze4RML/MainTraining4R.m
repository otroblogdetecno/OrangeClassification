% MainTraining4R.m
% 4R = 4 recortes, EM= eje menor
% extrae caracteristicas del tamano para las 4 vistas de la naranja.
...
%pathEntradaAprender=strcat(pathPrincipal,'inputToCalibrate/');
...
%% Nombres de archivos 
 archivoConfiguracion=strcat(pathConfiguracion,'20170801configuracion.xml');
 archivoCalibracion=strcat(pathConfiguracion,'20170801calibracion.xml');
 archivoVector=strcat(pathResultados,'aSize4RML.csv');
...
for n=1:size(listado)
    ...
    PrincipalProcesarImagen4R( corrida, pathPrincipal, pathEntradaAprender, pathConfiguracion, pathAplicacionAprender, pathResultados, banderaCalibracion, banderaRotar, nombreImagenP, archivoConfiguracion, archivoCalibracion, archivoVector );
end %
