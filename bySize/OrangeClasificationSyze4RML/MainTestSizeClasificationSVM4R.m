...
pathResultados=strcat(pathPrincipal,'output/clasSize4RML/');
...
% la eficacia
 ...
 nombreArchivoSetCompleto='aTSCSizeSVM4RML.csv';
 
 
 %Leer las 
 nombreArchivoTraining='aTSSizeSVM4RML.csv';
 nombreArchivoTest='aTSizeSVM4RML.csv';
 nombreArchivoScreen='screenSizeSVM4RML.txt'; 
...
formatSpec='%s%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%s'; 
...
% Precision, sensibilidad, especificidad, exactitud
...
%se cargan las etiquetas de clasificacion, el lugar 30 corresponde a la
%etiqueta de clasificacion
tablaDSTrainingClasificacion=tablaDSTraining(:,30);

% se cargan las caracteristicas que alimentaran al clasificador,
% corresponde a la etiqueta de eje menor 26 al 29
tablaDSTrainingCaracteristicas=tablaDSTraining(:,26:29);
...
% mismas
fprintf('Entrenando clasificador SVM Multiclases\n');
%Clasificador = fitcknn(arrayTrainingCaracteristicas,arrayTrainingClasificacion,'NumNeighbors',numeroVecinos,'Standardize',1);

Clasificador = fitcecoc(arrayTrainingCaracteristicas,arrayTrainingClasificacion);
...
% Se obtiene la caracteristica para comparar. 26 al 29 eje menor de los
% recortes
tablaDSTestComparar=tablaDSTest(:,26:29);
...
for indiceTest=1:totalArrayTest
    ...
    etiquetaComparar=char(table2array(tablaDSTest(indiceTest,30)));
    %
    ejeMenorCompararR1=char(table2array(tablaDSTest(indiceTest,26)));            
    ejeMenorCompararR2=char(table2array(tablaDSTest(indiceTest,27)));            
    ejeMenorCompararR3=char(table2array(tablaDSTest(indiceTest,28)));            
    ejeMenorCompararR4=char(table2array(tablaDSTest(indiceTest,29)));            
...
fprintf('\n ------------------------------- \n');
fprintf('\n RESULTADO GENERAL DEL ALGORITMO - SVM \n');
for(filas=1:1:totalFilas)
