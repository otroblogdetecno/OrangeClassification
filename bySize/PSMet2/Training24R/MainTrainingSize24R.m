...
% commas.csv and serves as input for the process
% classifier.
...
clc; clear all; close all;
 ...
 %%
 fileConfiguration=strcat(pathConfiguration,'20170916configuracion.xml'); % For initial coordinates in image processing
 fileCalibration=strcat(pathConfiguration,'20170916calibracion.xml'); % to indicate to the user in the final part the calibration
 fileVector=strcat(pathResults,'acm2SIZEC4R.csv'); %output file
...
list=dir(strcat(pathInputImages,'*.jpg'));
...
fprintf('Make a copy of the %s file and place the LABELS classified by the expert before running the classifier \n', fileVector);
fprintf('\n -------------------------------- \n');
